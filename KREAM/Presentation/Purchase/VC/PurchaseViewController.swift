...
class PurchaseViewController: UIViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        super.viewWillAppear(animated)  // 부모 메서드 호출 누락 방지
        tabBarController?.tabBar.isHidden = true // 탭바 숨기기
    }

    /// 뷰가 사라질 때 호출, 탭 바를 다시 표시
    override func viewWillDisappear(_ animated: Bool) {
        super.viewWillDisappear(animated)  // 부모 메서드 호출 누락 방지
        tabBarController?.tabBar.isHidden = false // 탭바 복원
    }
