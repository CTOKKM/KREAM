...
class LoginView: UIView {
    ...
    public lazy var kakaoLoginButton: UIButton = {
        ...
        btn.layer.borderWidth = 1  // 테두리 두께 설정 (필요하다면 추가)
        btn.layer.borderColor = UIColor.gray.cgColor  // borderColor를 CGColor로 변환하여 설정
        
        // 버튼 이미지 설정
        btn.setImage(UIImage(named: "kakaoIcon"), for: .normal)
        btn.imageEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: 10)
        btn.titleEdgeInsets = UIEdgeInsets(top: 0, left: 10, bottom: 0, right: -10)  // 텍스트 위치 조정
        btn.tintColor = .black // 버튼 이미지 색상 변경
